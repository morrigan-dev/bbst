...
type
  ...
  {
    Diese Klasse zeichnet den Servo12. Dabei stehen verschiedene Methoden für die jeweiligen Skins zur Verfügung, die
    den Servo12 unterschiedlich zeichnen.
...
constructor TServo12.Create(AOwner: TComponent; roboarmModel: TRoboterarmModel);
begin
  inherited Create(AOwner, roboarmModel);

  Self.servoModel := TServoModel.Create($00a08179, 35.0, 35.0, 35.0, 100.0, clRed, false, 140.0, 30.0);
  Self.servoModel.addObserver(roboarmModel);
  LOG := TLogLogger.GetLogger(TServo12);
end;

{
  Standard Destruktor, der diesen Servo und alle erstellten Ressourcen wieder freigibt.
}
destructor TServo12.Destroy;
begin
...
function TServo12.GetModel() : TServoModel;
begin
...
procedure TServo12.RenderSimple(obj : PGLUQuadric);
var slices : Integer;
    stacks  : Integer;
...
begin
  ...
  if(roboarmModel.GetSelectedServo = SERVO_12) then
  ...
  glPushMatrix();
      ...
      stacks := getStacks(width / 2);
      gluSphere(obj, width / 2, slices, stacks);
  glPopMatrix();
end;

{
  Zeichnet die detailierte Variante des Servo 1/2. Alle Größen sind als Konstanten fest vorgegeben.

  ~param obj Das QuadricObject, das zum Zeichnen des Elements benötigt wird und die Zeicheninformationen enthällt.
}
procedure TServo12.RenderExtended(obj: PGLUQuadric);
var P1 : TVector3d;
begin
  if(LOG.IsTraceEnabled()) then LOG.Trace('RenderExtended(obj)');

  // Rotiere um Servo 1 + 2
  glTranslatef(0, SHP_CENTER_RING_OUTER_RADIUS, 0);
  glPushMatrix();
    glRotatef(180, 0, 1, 0);
    p1[0] := 0; p1[1] := 0; p1[2] := 0;
    glTranslatef(-0.5, -1, -19);  // kleinere Positions Korrekturen
    drawServo(obj, p1, roboarmModel.GetSelectedServo() = SERVO_12);
    p1[0] := 0; p1[1] := 0; p1[2] := 50.3;
    drawServo(obj, p1, roboarmModel.GetSelectedServo() = SERVO_12);
  glPopMatrix();

  glRotatef(-90+servoModel.GetCurDegree(), 0, 0, 1);

  // Verschiebe Universum zur Seite für den ersten und zweiten Servo
  glPushMatrix();
    glRotatef(180, 0, 1, 0);
    P1[0] := 0; P1[1] := 0; P1[2] := -ASB10_BOTTOM_BOARD_WIDTH / 2 - ASB09_THICKNESS / 2;
    drawServoHornPlastic(gluNewQuadric, P1, roboarmModel.GetSelectedServo() = SERVO_12);
    P1[0] := 0; P1[1] := 0; P1[2] := ASB10_BOTTOM_BOARD_WIDTH / 2 + ASB09_THICKNESS / 2;
    drawServoHornPlastic(gluNewQuadric, P1, roboarmModel.GetSelectedServo() = SERVO_12);
  glPopMatrix();
end;
