...
uses
  ...
  Dialogs, StdCtrls, Math, ExtCtrls, Buttons, ComCtrls,

  // Eigene Klassen
  unitElement, unitResourceManager, unitTreeViewer;
...

{
  ...
  Stelle, an der das zu löschende Element sitzt.
...
begin
  ...
  begin
    ...
    begin
      ...
      ShowInfoBoxMsg('nodeReplaced', clGreen, [element^.Data, smallestElement^.Data]);

      smallestElement^.LS := element^.LS;
      if(element^.RS <> smallestElement) then  // Verhindere Endlosschleife wenn
        smallestElement^.RS := element^.RS;    // das kleinste Element der
                                               // rechte Sohn des Elements ist
      if(element = Self.root) then
      begin
        DeletePointer(Self.root);              // Ist das zu löschende Element
        Self.root := smallestElement;          // das root, so muss das kleinste
        Self.imgTreeViewer.SetRoot(Self.root); // Element zum root werden
      end
      else
      begin
        DeletePointer(element);
        if(smallestElement^.Data < parent^.Data) then
          parent^.LS := smallestElement
        else
          parent^.RS := smallestElement;
      end;

      if(Self.paintingMode) then
        Self.imgTreeViewer.HighlightElement(smallestElement, clYellow);
...
{
  Diese Methode liefert das kleinste Element eines Baumes und löst es komplett von dem Baum ab, damit es an eine
  andere Stelle eingefügt werden kann.
